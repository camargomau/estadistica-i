...
```{r}
ggplot(data = dat, mapping = aes(x = CPI, y = HDI)) + geom_point(colour = "blue")
```

Asignar un color a los puntos según `Region`:

```{r}
ggplot(data = dat, mapping = aes(x = CPI, y = HDI)) + geom_point(aes(colour = Region))
```

Manteniendo el color según `Region`, agranadr los puntos asignándoles un tamaño de 2:

```{r}
ggplot(data = dat, mapping = aes(x = CPI, y = HDI)) + geom_point(aes(colour = Region), size = 2)
```

Manteniendo el color según `Region`, asignar un tamaño según `HDI.Rank`:

```{r}
ggplot(data = dat, mapping = aes(x = CPI, y = HDI)) + geom_point(aes(colour = Region, size = HDI.Rank))
```
...
```{r, message = FALSE}
ggplot(data = dat, mapping = aes(x = CPI, y = HDI)) + geom_point() + geom_smooth()
```

Hacer la línea de suavizado menos suave:

```{r, message = FALSE}
ggplot(data = dat, mapping = aes(x = CPI, y = HDI)) + geom_point() + geom_smooth(span = 0.5)
```

Cambiar la línea de suavizado para que utilize un modelo linear para las predicciones:

```{r, message = FALSE}
ggplot(data = dat, mapping = aes(x = CPI, y = HDI)) + geom_point() + geom_smooth(span = 0.5, method = "lm")
```

Permitir que la línea de suavizado del paso anterior varíe a través de los niveles de `Region`:

```{r, message = FALSE}
ggplot(data = dat, mapping = aes(x = CPI, y = HDI, colour = Region, fill = Region)) + geom_point() + geom_smooth(span = 0.5, method = "lm")
```

Sobreponer una línea de suavizado LOESS sobre la gráfica de dispersión utilizando `geom_line()`:

```{r, message = FALSE}
ggplot(data = dat, mapping = aes(x = CPI, y = HDI)) + geom_point() + geom_line(stat = "smooth", method = "loess")
```
...
```{r}
ggplot(data = dat, mapping = aes(x = CPI, y = HDI, colour = Region)) + geom_point()
```

Modificar las escalas de las abscisas, ordenadas y los colores para que tengan nombres más entendibles:

```{r}
ggplot(data = dat, mapping = aes(x = CPI, y = HDI, colour = Region)) + geom_point() + scale_x_continuous(name = "Índice de Percepción de Corrupción (CPI)") + scale_y_continuous(name = "Índice de Desarrollo Humano (HDI)") + scale_colour_discrete(name = "Región")
```

Modificar la escala de colores para que utilice valores específicos de mi elección:

```{r}
ggplot(data = dat, mapping = aes(x = CPI, y = HDI, colour = Region)) + geom_point() + scale_x_continuous(name = "Índice de Percepción de Corrupción (CPI)") + scale_y_continuous(name = "Índice de Desarrollo Humano (HDI)") + scale_colour_manual(name = "Región", values = c("red", "magenta", "blue", "grey", "green", "orange"))
```
...
```{r}
ggplot(data = midwest, mapping = aes(x = area, y = log(poptotal))) + geom_point()
```
...
```{r}
scatter <- ggplot(data = midwest, mapping = aes(x = area, y = log(poptotal))) + geom_point(aes(colour = state, size = log(popdensity)), alpha = 0.3)
...
```{r, message = FALSE}
scatter_faceted_colour <- scatter_faceted + scale_colour_brewer(palette = "Set1")
...
```{r, message = FALSE}
scatter_faceted_themed <- scatter_faceted_colour + theme_bw() + theme(axis.title = element_text(colour = "blue", face = "bold"), strip.background = element_rect(fill = "yellow"))
